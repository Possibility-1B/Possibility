...
public class Title extends BasicGameState {
	...
	public void init(GameContainer gc, StateBasedGame sbg) throws SlickException {
		newGame = new Rectangle(Main.getScreenWidth() / 2 - 100, Main.getScreenHeight() / 2 - 100,
				200, 75);
		resume = new Rectangle(Main.getScreenWidth() / 2 - 100, Main.getScreenHeight() / 2 + 25,
				200, 75);
		controls = new Rectangle(Main.getScreenWidth() / 2 - 100, Main.getScreenHeight() / 2 + 150,
				200, 75);
		quit = new Rectangle(Main.getScreenWidth() / 2 - 100, Main.getScreenHeight() / 2 + 275, 200,
				75);
	...
	public void render(GameContainer gc, StateBasedGame sbg, Graphics g) throws SlickException {
		...
		if (Mouse.getX() > newGame.getMinX() && Mouse.getX() < newGame.getMaxX()
			...
			g.draw(newGame);
			g.setColor(Color.white);
			g.drawString("New Game", ((Main.getScreenWidth() / 2 - 40)),
		...
		if (Mouse.getX() > resume.getMinX() && Mouse.getX() < resume.getMaxX()
			...
			g.draw(resume);
			g.setColor(Color.white);
			g.drawString("Resume", ((Main.getScreenWidth() / 2 - 30)),
					(Main.getScreenHeight() / 2 + 50));
		} else {
			g.setColor(Color.gray);
			g.fill(resume);
			g.setColor(Color.white);
		...
		if (Mouse.getX() > controls.getMinX() && Mouse.getX() < controls.getMaxX()
			...
			g.draw(controls);
			g.setColor(Color.white);
			g.drawString("Controls", ((Main.getScreenWidth() / 2 - 40)),
		...
		if (Mouse.getX() > quit.getMinX() && Mouse.getX() < quit.getMaxX()
			...
			g.draw(quit);
			g.setColor(Color.white);
			g.drawString("Quit", ((Main.getScreenWidth() / 2 - 25)),
	...
	public void mousePressed(int button, int x, int y) {
		if (button == 0 && x > newGame.getMinX() && x < newGame.getMaxX() && y > newGame.getMinY()
				&& y < newGame.getMaxY()) {
		...
		if (button == 0 && x > resume.getMinX() && x < resume.getMaxX() && y > resume.getMinY()
				&& y < resume.getMaxY()) {
			sbg.enterState(Main.GAME_ID);
		}

...
public class TitleLeave extends BasicGameState {
	...
	public void init(GameContainer gc, StateBasedGame sbg) throws SlickException {
		leave = new Rectangle(Main.getScreenWidth() / 2 - 200, Main.getScreenHeight() / 2 - 100,
				400, 200);
	...
	public void render(GameContainer gc, StateBasedGame sbg, Graphics g) throws SlickException {
		...
		if (Mouse.getX() > yes.getMinX() && Mouse.getX() < yes.getMaxX()
			...
			g.draw(yes);
			g.setColor(Color.white);
			g.drawString("Yes", (Main.getScreenWidth() / 2 - 115),
					(Main.getScreenHeight() / 2 + 15));
		} else {
			g.setColor(Color.gray);
			g.fill(yes);
			g.setColor(Color.white);
	...
	public void mousePressed(int button, int x, int y) {
		if (button == 0 && x > yes.getMinX() && x < yes.getMaxX() && y > yes.getMinY()
				&& y < yes.getMaxY()) {
			Main.getGameContainer().exit();
		}
		if (button == 0 && x > no.getMinX() && x < no.getMaxX() && y > no.getMinY()
				&& y < no.getMaxY()) {
			sbg.enterState(Main.TITLE_ID);
		}
	}

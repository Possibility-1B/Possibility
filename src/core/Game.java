...
public class Game extends BasicGameState{	
	...
	private static Level levelSelect, level1, level2, level3, level4, level5, level6,
		level7, level8, credits, curLevel = null;
	...
	private void checkLevelSelect(){
		if(curLevel == levelSelect){
			if(p.getxPos() > LevelSelector.getLvl1().getMinX() &&  p.getxPos() < 
					LevelSelector.getLvl1().getMaxX()){
				curLevel = level1;
				p.playerReset();
			}
			...
			if(p.getxPos() > LevelSelector.getCredits().getMinX() &&  p.getxPos() < 
					LevelSelector.getCredits().getMaxX()){
				curLevel = credits;
			}
		}
	}

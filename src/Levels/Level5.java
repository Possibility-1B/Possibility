...
public class Level5 extends Level {
	...
	public void render(Graphics g) throws SlickException {
		...
		// door & key
		if (!keyGrabbed) {
			AnimationLoader.returnAnimation("key").draw(3200, Main.getScreenHeight() - 220, 180,
					180);
		}

		if (keyGrabbed) {
			if (doorTimer < 25) {
				AnimationLoader.returnAnimation("door").draw(3500, Main.getScreenHeight() - 290,
						150, 230);
			}

			if (doorTimer >= 25) {
				ImageLoader.returnImages("openDoor").draw(3500, Main.getScreenHeight() - 290, 150,
						230);
			}
		} else {
	...
	private void generateWorld() {
		// spike
		SpikeManager.createSpike(0, Main.getScreenHeight() - 60, Main.getScreenWidth() * 3, 100,
				true);
		...
		pushBlockWalls();
		// Floors
		TileManager.createBorder(1700, Main.getScreenHeight() / 2 - 100,
				1700 + Main.getScreenWidth() / 2 - 300, Main.getScreenHeight() / 2 - 100);
		TileManager.createBorder(1700, Main.getScreenHeight() / 2 + 100,
				1700 + Main.getScreenWidth() / 2 - 300, Main.getScreenHeight() / 2 + 100);
		// Walls
		TileManager.createBorder(1700, Main.getScreenHeight(), 1700,
				Main.getScreenHeight() / 2 + 100 + 2);
		TileManager.createBorder(1700, 0, 1700, Main.getScreenHeight() / 2 - 100 - 2);
		TileManager.createBorder(1700 + Main.getScreenWidth() / 2 - 300, Main.getScreenHeight(),
				1700 + Main.getScreenWidth() / 2 - 300, Main.getScreenHeight() / 2 + 100 + 2);
		TileManager.createBorder(1700 + Main.getScreenWidth() / 2 - 300, 0,
				1700 + Main.getScreenWidth() / 2 - 300, Main.getScreenHeight() / 2 - 100 - 2);
		// Tiles
		TileManager.createTile(1700, 0, Main.getScreenWidth() / 2 - 300,
				Main.getScreenHeight() / 2 - 100, "white");
		TileManager.createTile(1700, Main.getScreenHeight() / 2 + 100,
				Main.getScreenWidth() / 2 - 300, Main.getScreenHeight(), "white");
		// switches

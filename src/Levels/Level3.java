...
public class Level3 extends Level {
	...
	public void render(Graphics g) throws SlickException {
		...
		if (keyGrabbed) {
			if (doorTimer < 25) {
				AnimationLoader.returnAnimation("door").draw(getWidth() - 250,
						Main.getScreenHeight() - 290, 150, 230);
			}

			if (doorTimer >= 25) {
				ImageLoader.returnImages("openDoor").draw(getWidth() - 250,
						(int) (Main.getScreenHeight() - 290), 150, 230);
			}
		} else {
			ImageLoader.returnImages("door").draw(getWidth() - 250,
					(int) (Main.getScreenHeight() - 290), 150, 230);
		}
